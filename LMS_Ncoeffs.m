...
for ii =1:nsteps
    flippedx = xinput(ii:-1:max([1 ii-IRlength+1]));
    lengthflippedx = length(flippedx);
    if lengthflippedx < IRlength
        flippedx = [flippedx;zeros(IRlength-lengthflippedx,1)];
    end

    y_hat = sum(flippedx.*IRestimate);

    e =  youtput(ii) - y_hat ;
    errorhistory(ii) = e;

    IRestimate = IRestimate + alpha*e*flippedx;
end
